...
* =========================================== */
func ErrorLoggingWithStackTrace(rq *REQ.RequestData, err error, stackTrace bool) *ErrorResult {
...
////Errorの結果をレスポンス
* =========================================== */
func ErrorResponse(w http.ResponseWriter, rq *REQ.RequestData, err error, errStatus int, stackTrace bool) {
	...
	if errStatus >= 500 {
		LOG.ApiRequestLogging(rq, "", LOG.ERROR)
	} else {
	...
	//Error logging with stacktrace
	er := ErrorLoggingWithStackTrace(rq, errors.WithStack(fmt.Errorf("%d, %w", errStatus, err)), stackTrace)
